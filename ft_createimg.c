...
/*   Updated: 2021/07/19 22:28:47 by gsiddiqu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static unsigned int	ft_getpixel(t_img wall, t_point iter, t_point block_size, t_point img_size)
{
	t_point	pos;
	char	*dst;
...
static void	ft_resize(t_img img_o, t_point img_o_size, t_img *img_m, t_point img_m_size)
{
	t_point	iter;
	...
	while (iter.x < img_m_size.x)
	{
		...
		{	
			ft_mlx_pixel_put(img_m, iter.x, iter.y, ft_getpixel(img_o, iter, img_m_size, img_o_size));
			iter.y = iter.y + 1;
		}
		iter.x = iter.x + 1;
	}
...
void	ft_createimg(t_game *game, char *path, void **img)
{
	...
	img_o.img = mlx_png_file_to_image(game->mlx, path, &(img_o_size.x), &(img_o_size.y));
	img_o.addr = mlx_get_data_addr(img_o.img, &img_o.bits_per_pixel, &img_o.line_length, &img_o.endian);
	img_m.img = mlx_new_image(game->mlx, img_m_size.x, img_m_size.y);
	img_m.addr = mlx_get_data_addr(img_m.img, &img_m.bits_per_pixel, &img_m.line_length, &img_m.endian);
